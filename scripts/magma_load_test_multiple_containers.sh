...
run_load_test()
{
    ...
    do
        ...
        docker exec -it $CONTAINER_NAME python3 enbsim_cli.py -P attach --imsi $IMSI --key 00112233445566778899aabbccddeeff --opc 63bfa50ee6523365ff14c1f45f88737d
        sleep 0.2
    done
}
...
echo "######  Staring enbsim containers"
...
do
    ...
    (cd ../ && docker run --name $CONTAINER_NAME -v .:/usr/src/eNB --cap-add NET_ADMIN -p $EXPOSED_PORT:36412 --net enbsim-net --ip $CONTAINER_ADDRESS  -d enbsim --enb_ip $CONTAINER_ADDRESS --mme_ip ${MME_IPS[(($i-1))]})
    sleep 0.2
done
...
do
    ...
    docker exec -it $CONTAINER_NAME python3 enbsim_cli.py -P s1-setup --enbname $CONTAINER_NAME --mcc 315 --mnc 010 --enbid $ENB_ID --tac1 1

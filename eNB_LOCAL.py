...
from enbsim_defs import ENBSIM_QUEUE_ID, S1AP_PORT
...
def print_log(session_dict, log_message):
    if 'IMSI' in session_dict:
        logger.info(f"{session_dict['IMSI']} {log_message}")
    return session_dict
...
def add_ns(ns_name, veth, nseth, ue_ip):
    ...
    if ns_name in sys_ns:
        if os.system(f"ip netns del {ns_name}") != 0:
            logger.error(f"Error deleting namespace {ns_name} ++++++++++++")
    if os.system(f"ip netns add {ns_name}") != 0:
        logger.error(f"Error adding namespace {ns_name} -------------")
    ...
    if veth in veth_exists and nseth in veth_exists:
        logger.info(f"*************** {veth} added successfully")
    ...
    os.system(
        f"ip netns exec {ns_name} ip route add 169.254.1.1 dev {nseth} scope link")
    os.system(
        f"ip netns exec {ns_name} ip route add default via 169.254.1.1 dev {nseth}")
    os.system(f"ip netns exec {ns_name} arp -s 169.254.1.1 ee:ee:ee:ee:ee:ee")
    os.system(
        f"ip netns exec {ns_name} ln -fs /etc/resolvconf/resolv.conf.d/head /etc/resolv.conf")
    ...
    mac_addr = str(os.popen(
        f"ip netns exec {ns_name} cat /sys/class/net/{nseth}/address").read()).strip()
    global down_gtp
...
def session_dict_initialization(session_dict):
    ...
    session_dict['NAS-KEY-EEA1'] = return_key(
        session_dict['KASME'], 1, 'NAS-ENC')
    session_dict['NAS-KEY-EEA2'] = return_key(
        session_dict['KASME'], 2, 'NAS-ENC')
    session_dict['NAS-KEY-EEA3'] = return_key(
        session_dict['KASME'], 3, 'NAS-ENC')
    session_dict['NAS-KEY-EIA1'] = return_key(
        session_dict['KASME'], 1, 'NAS-INT')
    session_dict['NAS-KEY-EIA2'] = return_key(
        session_dict['KASME'], 2, 'NAS-INT')
    session_dict['NAS-KEY-EIA3'] = return_key(
        session_dict['KASME'], 3, 'NAS-INT')
    ...
    if session_dict['LOCAL_KEYS'] == True:
        ...
        if session_dict['IMSI'] == None:
            try:
                session_dict['IMSI'] = return_imsi(
                    session_dict['SERIAL-INTERFACE'])
                if session_dict['IMSI'] == None:
    ...
    session_dict['ENCODED-GUTI'] = eNAS.encode_guti(
        session_dict['PLMN'], 32769, 1, 12345678)
    ...
    session_dict['NON-IP-PACKETS'] = [NON_IP_PACKET_1,
                                      NON_IP_PACKET_2, NON_IP_PACKET_3, NON_IP_PACKET_4]
...
def return_kasme(plmn, autn, ck, ik):
    ...
    message = unhexlify('10') + return_plmn(plmn) + \
        unhexlify('0003' + sqn_xor_ak + '0006')
    h = HMAC.new(key, msg=message, digestmod=SHA256)
    return h.digest()[-32:]
...
def get_imsi(serial_interface):
    ...
    ser = serial.Serial(serial_interface, 38400, timeout=0.5,
                        xonxoff=True, rtscts=True, dsrdtr=True, exclusive=True)
...
def get_res_ck_ik(serial_interface, rand, autn):
    ...
    CLI.append('AT+CSIM=78,\"008800812210' +
               rand.upper() + '10' + autn.upper() + '\"\r\n')
...
def read_res_ck_ik(reader_index, rand, autn):
    ...
    data, sw1, sw2 = connection.transmit(
        toBytes('00A4040010A0000000871002FFFFFFFF8903050001'))
    data, sw1, sw2 = connection.transmit(
...
def https_res_ck_ik(server, rand, autn):
    r = requests.get('https://' + server + '/?type=rand-autn&rand=' +
                     rand + '&autn=' + autn, verify=False)
...
def S1SetupRequest(dic):
    ...
    IEs = []
    IEs.append({'id': 59, 'value': ('Global-ENB-ID', {'pLMNidentity': dic['ENB-PLMN'], 'eNB-ID': (
        'homeENB-ID', (dic['ENB-ID'], 28))}), 'criticality': 'reject'})
    IEs.append({'id': 60, 'value': (
        'ENBname', dic['ENB-NAME']), 'criticality': 'ignore'})
    if dic['S1-TYPE'] == "4G":
        IEs.append({'id': 64, 'value': ('SupportedTAs', [
                   {'tAC': dic['ENB-TAC1'], 'broadcastPLMNs': [dic['ENB-PLMN']]}]), 'criticality': 'reject'})
    elif dic['S1-TYPE'] == "NBIOT":
        IEs.append({'id': 64, 'value': ('SupportedTAs', [{'tAC': dic['ENB-TAC-NBIOT'], 'broadcastPLMNs': [dic['ENB-PLMN']], 'iE-Extensions': [
                   {'id': 232, 'criticality': 'reject', 'extensionValue': ('RAT-Type', 'nbiot')}]}]), 'criticality': 'reject'})
    elif dic['S1-TYPE'] == "BOTH":
        IEs.append({'id': 64, 'value': ('SupportedTAs', [{'tAC': dic['ENB-TAC'], 'broadcastPLMNs': [dic['ENB-PLMN']]}, {'tAC': dic['ENB-TAC-NBIOT'], 'broadcastPLMNs': [
                   dic['ENB-PLMN']], 'iE-Extensions': [{'id': 232, 'criticality': 'reject', 'extensionValue': ('RAT-Type', 'nbiot')}]}]), 'criticality': 'reject'})
    IEs.append({'id': 137, 'value': (
        'PagingDRX', 'v128'), 'criticality': 'ignore'})
    if dic['S1-TYPE'] == "NBIOT" or dic['S1-TYPE'] == "BOTH":
        IEs.append({'id': 234, 'value': (
            'NB-IoT-DefaultPagingDRX', 'v256'), 'criticality': 'ignore'})
    val = ('initiatingMessage', {'procedureCode': 17, 'value': (
        'S1SetupRequest', {'protocolIEs': IEs}), 'criticality': 'reject'})
...
def PathSwitchRequest(dic, user_dict):
    IEs = []
    IEs_RABs_List = []
    IEs.append({'id': 8, 'value': ('ENB-UE-S1AP-ID',
               dic['ENB-UE-S1AP-ID']), 'criticality': 'reject'})
    for rab in range(len(dic['e_RAB_id'])):
        e_RAB_id = dic['e_RAB_id'][rab]
        IEs_RAB = {'id': 23, 'value': ('E-RABToBeSwitchedDLItem', {'e-RAB-ID': e_RAB_id, 'transportLayerAddress': (
            dic['ENB-GTP-ADDRESS-INT'], 32), 'gTP-TEID': (struct.pack('>I', upteid_get()+100000))}), 'criticality': 'ignore'}
        IEs_RABs_List.append(IEs_RAB)
    IEs.append({'id': 22, 'value': ('E-RABToBeSwitchedDLList',
               IEs_RABs_List), 'criticality': 'ignore'})
    IEs.append({'id': 88, 'value': ('MME-UE-S1AP-ID',
               dic['MME-UE-S1AP-ID']), 'criticality': 'reject'})
    IEs.append({'id': 100, 'value': ('EUTRAN-CGI', {'cell-ID': (
        dic['ENB-CELLID'], 28), 'pLMNidentity': dic['ENB-PLMN']}), 'criticality': 'ignore'})
    IEs.append({'id': 67, 'value': ('TAI', {
               'pLMNidentity': dic['ENB-PLMN'], 'tAC': dic['ENB-TAC']}), 'criticality': 'reject'})
    IEs.append({'id': 107, 'value': ('UESecurityCapabilities', {'encryptionAlgorithms': (
        57344, 16), 'integrityProtectionAlgorithms': (57344, 16)}), 'criticality': 'ignore'})
    IEs.append({'id': 157, 'value': ('GUMMEI', {
               'pLMN-Identity': dic['ENB-PLMN'], 'mME-Group-ID': user_dict['enb']['MME-GROUP-ID'], 'mME-Code': user_dict['enb']['MME-CODE']}), 'criticality': 'ignore'})
    val = ('initiatingMessage', {'procedureCode': 3, 'value': (
        'PathSwitchRequest', {'protocolIEs': IEs}), 'criticality': 'reject'})
...
def MMEConfigurationUpdateAcknowledge(IEs, dic):
    ...
    answer = ('successfulOutcome', {'procedureCode': 30, 'value': (
        'MMEConfigurationUpdateAcknowledge', {'protocolIEs': []}), 'criticality': 'ignore'})
...
def ENBConfigurationUpdate(dic, users):
    IEs = []
    IEs.append({'id': 60, 'value': ('ENBname', 'newname'),
               'criticality': 'ignore'})
    IEs.append({'id': 64, 'value': ('SupportedTAs', [{'tAC': int(63).to_bytes(2, byteorder='big'), 'broadcastPLMNs': [return_plmn_s1ap(
        '315010')]}, {'tAC': int(69).to_bytes(2, byteorder='big'), 'broadcastPLMNs': [return_plmn_s1ap('315011')]}]), 'criticality': 'reject'})
    IEs.append({'id': 137, 'value': (
        'PagingDRX', 'v32'), 'criticality': 'ignore'})
    val = ('initiatingMessage', {'procedureCode': 29, 'value': (
        'ENBConfigurationUpdate', {'protocolIEs': IEs}), 'criticality': 'reject'})
...
def Reset(dic, users):
    ...
    IEs = []
    IEs.append({'id': 2, 'value': (
        'Cause', ('misc', 'om-intervention')), 'criticality': 'ignore'})
    id_pair = []
    for user in users:
        if user != 'enb':
            final_ie = {'id': 91, 'value': ('UE-associatedLogicalS1-ConnectionItem', {
                                            'mME-UE-S1AP-ID': users[user]['MME-UE-S1AP-ID'], 'eNB-UE-S1AP-ID': users[user]['ENB-UE-S1AP-ID']}), 'criticality': 'ignore'}
            id_pair.append(final_ie)
    if len(id_pair) == 0:
        IEs.append({'id': 92, 'value': (
            'ResetType', ('s1-Interface', 'reset-all')), 'criticality': 'ignore'})
    else:
        IEs.append({'id': 92, 'value': (
            'ResetType', ('partOfS1-Interface', id_pair)), 'criticality': 'ignore'})

    val = ('initiatingMessage', {'procedureCode': 14, 'value': (
        'Reset', {'protocolIEs': IEs}), 'criticality': 'ignore'})
        ...
        if i['id'] == 92:
            for j in (i['value'][1][1]):
                ...
                if 'mME-UE-S1AP-ID' in id_pair and 'eNB-UE-S1AP-ID' in id_pair:
                    IEs_pair_List.append({'id': 91, 'value': ('UE-associatedLogicalS1-ConnectionItem', {
                                         'mME-UE-S1AP-ID': id_pair['mME-UE-S1AP-ID'], 'eNB-UE-S1AP-ID': id_pair['eNB-UE-S1AP-ID']}), 'criticality': 'ignore'})
    IE.append({'id': 93, 'value': (
        'UE-associatedLogicalS1-ConnectionListResAck', IEs_pair_List), 'criticality': 'ignore'})
    answer = ('successfulOutcome', {'procedureCode': 14, 'value': (
        'ResetAcknowledge', {'protocolIEs': IE}), 'criticality': 'ignore'})
...
def Overload(dic):
    ...
    val = ('initiatingMessage', {'procedureCode': 35, 'value': (
        'OverloadStop', {'protocolIEs': IEs}), 'criticality': 'ignore'})
...
def nas_pdn_disconnect(eps_bearer_identity, pti, linked_eps_bearer_id, pco):
    ...
    esm_list.append((2, eps_bearer_identity))
    esm_list.append((0, 'V', bytes([pti])))  # procedure trnasaction identity
    # message type: pdn disconnectrequest
    esm_list.append((0, 'V', bytes([210])))
    # pdn type / request type (ipv4, initial request)
    esm_list.append((0, 'V', bytes([linked_eps_bearer_id])))
...
def nas_activate_default_eps_bearer_context_accept(eps_bearer_identity, pco):
    ...
    esm_list.append((2, eps_bearer_identity))
    esm_list.append((0, 'V', bytes([pti])))  # procedure trnasaction identity
    # message type: activate_default_eps_bearer_context_accept
    esm_list.append((0, 'V', bytes([194])))
    if pco != None:
        esm_list.append((0x27, 'TLV', pco))
    return eNAS.nas_encode(esm_list)
...
def nas_activate_dedicated_eps_bearer_context_accept(eps_bearer_identity, pco):
    ...
    esm_list.append((2, eps_bearer_identity))
    esm_list.append((0, 'V', bytes([pti])))  # procedure trnasaction identity
    # message type: activate_dedicated_eps_bearer_context_accept
...
def nas_esm_information_response(eps_bearer_identity, pti, apn, pco):
    ...
    esm_list.append((2, eps_bearer_identity))
    esm_list.append((0, 'V', bytes([pti])))  # procedure trnasaction identity
    # message type: eesm information response
    esm_list.append((0, 'V', bytes([218])))
    if apn != None:
        esm_list.append((0x28, 'TLV', apn))
    if pco != None:
        esm_list.append((0x27, 'TLV', pco))
...
def nas_attach_request(type, esm_information_transfer_flag, eps_identity, pdp_type, attach_type, tmsi, lai, sms_update, pcscf_restoration, guti=None, ksi=0):
    ...
    # eps attach type/ nas key set identifier (EPS Attach /Keyset 0)
    emm_list.append((0, 'V', bytes([(ksi << 4) + attach_type])))
    if guti == None:
        # eps mobile identity (imsi/odd number:9) + imsi. all in bcd)
    ...
    if attach_type == 6:  # EPS Emergency Attach
        emm_list.append((0, 'LV-E', nas_pdn_connectivity(0, 1,
                        pdp_type, None, pco, esm_information_transfer_flag, 4)))
    else:
        emm_list.append((0, 'LV-E', nas_pdn_connectivity(0, 1,
                        pdp_type, None, pco, esm_information_transfer_flag)))
    if guti != None:
    ...
    if type[0] == "4G":
        ...
        if attach_type == 2 and tmsi != None:
            emm_list.append(
                (0x10, 'TLV', tmsi[-3:-2] + bytes([(tmsi[-2]//64)*64])))
...
def nas_tracking_area_update_request(ksi, eps_update_type, eps_identity, type, tmsi, lai, sms_update):
    ...
    emm_list.append((0, 'V', bytes([72])))
    # ksi=6, update type: TA
    emm_list.append((0, 'V', bytes([(ksi << 4) + eps_update_type])))
    # eps mobile identity (imsi/odd number:9) + imsi. all in bcd)
    emm_list.append((0, 'LV', eps_identity))
    if type[0] == "4G":
...
def nas_extended_service_request(ksi, mobile_identity):
    emm_list = []
    emm_list.append((7, 0))  # protocol discriminator /
    # message type: extende service request
    emm_list.append((0, 'V', bytes([76])))
    # mobile terminating cs fallback
    emm_list.append((0, 'V', bytes([(ksi << 4) + 1])))
...
def nas_detach_request(ksi, detach_type, eps_identity):
    ...
    # ksi=6, update type: TA
    emm_list.append((0, 'V', bytes([(ksi << 4) + detach_type])))
    # eps mobile identity (imsi/odd number:9) + imsi. all in bcd)
    emm_list.append((0, 'LV', eps_identity))
    return eNAS.nas_encode(emm_list)
...
def nas_identity_response(imsi_or_imeisv):
    ...
    # eps mobile identity (imsi/odd number:9) + imsi. all in bcd)
    emm_list.append((0, 'LV', bcd(imsi_or_imeisv)))
    return eNAS.nas_encode(emm_list)
...
def nas_attach_complete(eps_bearer_identity):
    ...
    emm_list.append(
        (0, 'LV-E', nas_activate_default_eps_bearer_context_accept(eps_bearer_identity, None)))
    return eNAS.nas_encode(emm_list)
...
def nas_security_protected_nas_message(security_header, message_authentication_code, sequence_number, nas_message):
    ...
    # message type: authentication response
    emm_list.append((0, 'V', message_authentication_code))
...
def nas_service_request(security_header, ksi, sequence_number, message_authentication_code):
    ...
    # message type: authentication response
    emm_list.append(
        (0, 'V', bytes([((ksi << 5) & 0xff) + (sequence_number & 0x1f)])))
    # message type: authentication response
    emm_list.append((0, 'V', message_authentication_code))
...
def nas_control_plane_service_request(ksi, control_plane_service_type, esm_message_container, nas_message_container, eps_bearer_context_status):
    ...
    emm_list.append((0, 'V', bytes([77])))
    emm_list.append(
        (0, 'V', bytes([((ksi << 4) & 0xf0) + (control_plane_service_type & 0x0f)])))
...
def nas_uplink_nas_transport(sms):
    emm_list = []
    emm_list.append((7, 0))  # protocol discriminator /
    # message type: uplink_nas_transport
    emm_list.append((0, 'V', bytes([99])))
...
def ProcessUplinkNAS(message_type, dic):
    ...
    if message_type == 'service request':
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        # use first 2 bytes to calculate integrity 24.301 9.9.3.28
        dic['NAS-ENC'] = nas_service_request(
            12, dic['NAS-KEY-SET-IDENTIFIER'], dic['UP-COUNT'] % 256, b'\x00\x00')[0:2]
        ...
        dic['NAS-ENC'] = nas_extended_service_request(
            dic['NAS-KEY-SET-IDENTIFIER'], dic['S-TMSI'][1:5])
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0

        mac_bytes = nas_hash(dic)
        dic['NAS'] = nas_security_protected_nas_message(
            1, mac_bytes, bytes([dic['UP-COUNT'] % 256]), dic['NAS-ENC'])
        ...
        dic['NAS-ENC'] = nas_tracking_area_update_request(dic['NAS-KEY-SET-IDENTIFIER'], dic['TAU-TYPE'], dic['GUTI'], (
            dic['SESSION-TYPE'], dic['SESSION-SESSION-TYPE']), dic['TMSI'], dic['LAI'], dic['SMS-UPDATE-TYPE'])
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        ...
        dic['NAS-ENC'] = nas_tracking_area_update_request(dic['NAS-KEY-SET-IDENTIFIER'], 3, dic['GUTI'], (
            dic['SESSION-TYPE'], dic['SESSION-SESSION-TYPE']), dic['TMSI'], dic['LAI'], dic['SMS-UPDATE-TYPE'])
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        ...
        dic['NAS-ENC'] = nas_tracking_area_update_request(dic['NAS-KEY-SET-IDENTIFIER'], 2, dic['GUTI'], (
            dic['SESSION-TYPE'], dic['SESSION-SESSION-TYPE']), dic['TMSI'], dic['LAI'], dic['SMS-UPDATE-TYPE'])
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        ...
        # normal detach (0---). EPS detach (-001)
        dic['NAS-ENC'] = nas_detach_request(
            dic['NAS-KEY-SET-IDENTIFIER'], 1, dic['GUTI'])
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        ...
        if dic['MME-UE-S1AP-ID'] > 0:  # s1 up
            dic['NAS'] = nas_security_protected_nas_message(
                2, mac_bytes, bytes([dic['UP-COUNT'] % 256]), dic['NAS-ENC'])
        else:
            dic['NAS'] = nas_security_protected_nas_message(1, mac_bytes, bytes(
                [dic['UP-COUNT'] % 256]), dic['NAS-ENC'])  # com s1 em baixo vai no initialuemessage so com integrity
        ...
        if dic['ATTACH-TYPE'] == 6:  # If Attach Type = EPS Emergency PDN Connectity is with Emergency APN
            dic['NAS-ENC'] = nas_pdn_connectivity(0,
                                                  2, dic['PDP-TYPE'], None, pco, None, 4)
        else:
            dic['NAS-ENC'] = nas_pdn_connectivity(
                0, 2, dic['PDP-TYPE'], eNAS.encode_apn(APN), pco, None)
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        nas_encrypted = nas_encrypt(dic)
        dic['NAS-ENC'] = nas_encrypted
        ...
        dic['NAS-ENC'] = nas_pdn_disconnect(0, 2,
                                            dic['EPS-BEARER-IDENTITY'][-1], None)
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        nas_encrypted = nas_encrypt(dic)
        dic['NAS-ENC'] = nas_encrypted
        ...
        dic['NAS-ENC'] = nas_control_plane_service_request(
            dic['NAS-KEY-SET-IDENTIFIER'], dic['CPSR-TYPE'], None, None, None)
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        # nas_encrypted = nas_encrypt(dic)
        # dic['NAS-ENC'] = nas_encrypted
        mac_bytes = nas_hash(dic)
        dic['NAS'] = nas_security_protected_nas_message(5, mac_bytes, bytes(
            [dic['UP-COUNT'] % 256]), dic['NAS-ENC'])  # 5 especifico para esta mensagem
        ...
        dic['NAS-ENC'] = nas_control_plane_service_request(
            dic['NAS-KEY-SET-IDENTIFIER'], dic['CPSR-TYPE'], dic['NAS-ENC'], None, b'\x20\x00')
        ...
        dic['NAS-ENC'] = nas_esm_data_transport(
            dic['EPS-BEARER-IDENTITY'][-1], 0, dic['USER-DATA-CONTAINER'])
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        nas_encrypted = nas_encrypt(dic)
        dic['NAS-ENC'] = nas_encrypted
        mac_bytes = nas_hash(dic)
        dic['NAS'] = nas_security_protected_nas_message(2, mac_bytes, bytes(
            [dic['UP-COUNT'] % 256]), dic['NAS-ENC'])  # mudei de 4 para 2
...
def ProcessDownlinkNAS(dic):
    ...
    if nas_list[0][1] == 7:
        message_type = nas_list[2][1]
    if nas_list[0][1] == 2:
        message_type = nas_list[3][1]
    ...
    if message_type == 82:  # authentication request
        ...
        if dic['LOCAL_KEYS'] == True:
            ...
            if dic['LOCAL_MILENAGE'] == True:
                res, ck, ik = milenage_res_ck_ik(
                    dic['KI'], dic['OP'], dic['OPC'], rand)  # no sqn validation
            else:
                res, ck, ik = return_res_ck_ik(
                    dic['SERIAL-INTERFACE'], rand, autn)
            if res is not None and ck is not None and ik is not None:
                ...
                dic['XRES'] = unhexlify(res)
            if dic['AUTH-ERROR']:
                dic['NAS'] = nas_nondelivery_Indication(
                    unhexlify("00"+rand), unhexlify(autn))
            else:
                dic['NAS'] = nas_authentication_response(dic['XRES'])
        ...
        dic['NAS'] = nas_security_protected_nas_message(
            4, mac_bytes, bytes([dic['UP-COUNT'] % 256]), dic['NAS-ENC'])
            ...
            if i[0] == 'esm message container':
                ...
                for m in i[1]:
                    ...
                    if m[0] == 'eps bearer identity':
                        ...
                        pdn_address = eNAS.decode_pdn_address(
                            dic['PDN-ADDRESS'][position])
                        ...
                        for x in pdn_address:
                            if x[0] == 'ipv4':
                                ...
                                add_ns(dic['IMSI'], dic['UE-NAMESPACE'][0],
                                       dic['UE-NAMESPACE'][1], dic['PDN-ADDRESS-IPV4'])
                                # try:
                                ...
                                if dic['PDN-ADDRESS-IPV6'] is not None:
                                    subprocess.call("ip -6 addr del " + dic['PDN-ADDRESS-IPV6'] + "/64 dev tun" + str(
                                        dic['SESSION-TYPE-TUN']), shell=True)
                                dic['PDN-ADDRESS-IPV6'] = x[1]
                                subprocess.call(
                                    "ip -6 addr add " + x[1] + "/64 dev tun" + str(dic['SESSION-TYPE-TUN']), shell=True)

                    elif m[0] == 'access point name':

                        dic['EPS-BEARER-APN'][position] = m[1]
                        dic = print_log(dic, eNAS.decode_apn(
                            dic['EPS-BEARER-APN'][position]))
                ...
                dic = print_log(
                    dic, eNAS.decode_eps_mobile_identity(dic['GUTI']))
        ...
        dic['NAS-ENC'] = nas_attach_complete(
            dic['EPS-BEARER-IDENTITY'][position])
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        gtp_dict[dic['GTP-KEY']] = ((dic['SGW-TEID'])
                                    [-1], (dic['SGW-GTP-ADDRESS'])[-1])
        nas_encrypted = nas_encrypt(dic)
        dic['NAS-ENC'] = nas_encrypted
        mac_bytes = nas_hash(dic)
        dic['NAS'] = nas_security_protected_nas_message(2, mac_bytes, bytes(
        ...
        if len(dic['SGW-GTP-ADDRESS']) > 0:
            ...
            dic = print_log(
                dic, "GTP-U: Deactivation due to DetachRequest received")
        ...
        if len(dic['SGW-GTP-ADDRESS']) > 0:
            ...
            # os.write(dic['PIPE-OUT-GTPU-DECAPSULATE'],b'\x02' + dic['SGW-GTP-ADDRESS'][-1] + b'\x00\x00\x00' + bytes([dic['RAB-ID'][-1]]))
            dic = print_log(
                dic, "GTP-U: Deactivation due to DetachAccept received")
        ...
        for i in nas_list:
            if i[0] == 'guti':
                dic = print_log(
                    dic, str(eNAS.decode_eps_mobile_identity(dic['GUTI'])))
                if dic['GUTI'] != i[1] and dic['NAS'] == None:  # new GUTI assigned
                    ...
                    dic['NAS'] = nas_security_protected_nas_message(2, mac_bytes, bytes(
                        [dic['UP-COUNT'] % 256]), dic['NAS-ENC'])  # mudei de 4 para 2
                    dic = print_log(
                        dic, "NAS: sending trackingAreaUpdateComplete")
                dic['GUTI'] = i[1]
                dic['S-TMSI'] = i[1][-5:]
        ...
        for i in nas_list:
            if i[0] == 'identity type':
                if i[1] == 1:  # imsi
                    ...
                    dic = print_log(
                        dic, "NAS: sending IdentityResponse (IMSI)")
                elif i[1] == 3:  # imeisv
                    dic['NAS'] = nas_identity_response(
                        '3' + dic['IMEISV'] + 'f')
                    dic = print_log(
                        dic, "NAS: sending IdentityResponse (IMEI-SV)")
            ...
            if i[0] == "nas message container":
                ...
                if i[1][1:2] != b'\x04' and i[1][1:2] != b'\x10':
                    ...
                    if i[1][0] < 128:
                        ...
                        dic['NAS-ENC'] = nas_encrypted
                        mac_bytes = nas_hash(dic)
                        dic['NAS-SMS-MT'] = nas_security_protected_nas_message(2, mac_bytes, bytes(
                            [dic['UP-COUNT'] % 256]), dic['NAS-ENC'])  # mudei de 4 para 2
                        dic = print_log(dic, "NAS: sending UplinkNASTransport")
...
#### ESM #####
        ...
        dic = print_log(
            dic, "NAS: ActivateDefaultEPSbearerContextRequest received")
        for i in nas_list:
        ...
        dic['NAS-ENC'] = nas_activate_default_eps_bearer_context_accept(
            dic['EPS-BEARER-IDENTITY'][position], None)
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        nas_encrypted = nas_encrypt(dic)
        dic['NAS-ENC'] = nas_encrypted
        ...
        dic['NAS'] = nas_security_protected_nas_message(2, mac_bytes, bytes(
            [dic['UP-COUNT'] % 256]), dic['NAS-ENC'])  # mudei de 4 para 2
        dic = print_log(
            dic, "NAS: sending ActivateDefaultEPSbearerContextAccept")

    elif message_type == 197:
        dic = print_log(
            dic, "NAS: ActivateDedicatedEPSbearerContextRequest received")
        for i in nas_list:
            ...
            if i[0] == 'eps bearer identity':
                ...
                position = dic['EPS-BEARER-IDENTITY'].index(i[1])
        dic['NAS-ENC'] = nas_activate_dedicated_eps_bearer_context_accept(
            dic['EPS-BEARER-IDENTITY'][position], None)
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        nas_encrypted = nas_encrypt(dic)
        ...
        dic['NAS'] = nas_security_protected_nas_message(2, mac_bytes, bytes(
            [dic['UP-COUNT'] % 256]), dic['NAS-ENC'])  # mudei de 4 para 2
        dic = print_log(
            dic, "NAS: sending ActivateDedicatedEPSbearerContextAccept")
        ...
        dic['NAS-ENC'] = nas_deactivate_eps_bearer_context_accept(
            bearer, 0, None)
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        nas_encrypted = nas_encrypt(dic)
        ...
        dic['NAS-ENC'] = nas_esm_information_response(
            0, 1, return_apn(dic['APN']), None)
        dic['UP-COUNT'] += 1
        dic['DIR'] = 0
        nas_encrypted = nas_encrypt(dic)
        dic['NAS-ENC'] = nas_encrypted
        ...
        dic = print_log(dic, "NAS: MessageType =" +
                        str(message_type) + " received")
        dic['NAS'] = None
...
def InitialUEMessage(dic, msg=None):
    ...
    IEs.append({'id': 8, 'value': ('ENB-UE-S1AP-ID',
               dic['ENB-UE-S1AP-ID']), 'criticality': 'reject'})
    IEs.append({'id': 26, 'value': (
        'NAS-PDU', dic['NAS']), 'criticality': 'reject'})
    if dic['SESSION-TYPE'] == "4G" or dic['SESSION-TYPE'] == "5G":
        IEs.append({'id': 67, 'value': ('TAI', {
                   'pLMNidentity': dic['ENB-PLMN'], 'tAC': dic['ENB-TAC']}), 'criticality': 'reject'})
    elif dic['SESSION-TYPE'] == "NBIOT":
        IEs.append({'id': 67, 'value': ('TAI', {
                   'pLMNidentity': dic['ENB-PLMN'], 'tAC': dic['ENB-TAC-NBIOT']}), 'criticality': 'reject'})

    IEs.append({'id': 100, 'value': ('EUTRAN-CGI', {'cell-ID': (
        dic['ENB-CELLID'], 28), 'pLMNidentity': dic['ENB-PLMN']}), 'criticality': 'ignore'})
    if dic['ATTACH-TYPE'] == 6:  # emergency
        IEs.append({'id': 134, 'value': ('RRC-Establishment-Cause',
                   'emergency'), 'criticality': 'ignore'})
    else:
        IEs.append({'id': 134, 'value': ('RRC-Establishment-Cause',
                   'mo-Signalling'), 'criticality': 'ignore'})

    if dic['S-TMSI'] != None:
        IEs.append({'id': 96, 'value': (
            'S-TMSI', {'mMEC': dic['S-TMSI'][0:1], 'm-TMSI': dic['S-TMSI'][1:5]}), 'criticality': 'reject'})
    if 'GUTI' in dic and msg != None:
        IEs.append({'id': 170, 'value': (
            'GUMMEIType', 'native'), 'criticality': 'ignore'})
#    IEs.append({'id': 75, 'value': ('GUMMEI', {'pLMN-Identity': dic['ENB-PLMN'], 'mME-Group-ID': dic['MME-GROUP-ID'], 'mME-Code': dic['MME-CODE']}), 'criticality': 'reject'})
    ...
    val = ('initiatingMessage', {'procedureCode': 12, 'value': (
        'InitialUEMessage', {'protocolIEs': IEs}), 'criticality': 'ignore'})
...
def UplinkNASTransport(dic):
    ...
    IEs = []
    IEs.append({'id': 0, 'value': ('MME-UE-S1AP-ID',
               dic['MME-UE-S1AP-ID']), 'criticality': 'reject'})
    IEs.append({'id': 8, 'value': ('ENB-UE-S1AP-ID',
               dic['ENB-UE-S1AP-ID']), 'criticality': 'reject'})
    IEs.append({'id': 26, 'value': (
    ...
    if dic['SESSION-TYPE'] == "4G" or dic['SESSION-TYPE'] == "5G":
        IEs.append({'id': 67, 'value': ('TAI', {
                   'pLMNidentity': dic['ENB-PLMN'], 'tAC': dic['ENB-TAC']}), 'criticality': 'ignore'})
    elif dic['SESSION-TYPE'] == "NBIOT":
        IEs.append({'id': 67, 'value': ('TAI', {
                   'pLMNidentity': dic['ENB-PLMN'], 'tAC': dic['ENB-TAC-NBIOT']}), 'criticality': 'ignore'})

    val = ('initiatingMessage', {'procedureCode': 13, 'value': (
        'UplinkNASTransport', {'protocolIEs': IEs}), 'criticality': 'ignore'})
...
def ProcessLocationReportingControl(IEs, dic):
    ...
    val = ('initiatingMessage', {'procedureCode': 33, 'value': (
        'LocationReport', {'protocolIEs': IEs}), 'criticality': 'ignore'})
    ...
    if dic['NAS'] != None or dic['NAS-SMS-MT'] != None:
        if dic['NAS'] != None:
            ...
            if dic['AUTH-ERROR']:
                ...
                IEs.append({'id': 8, 'value': ('ENB-UE-S1AP-ID',
                           dic['ENB-UE-S1AP-ID']), 'criticality': 'reject'})
                IEs.append({'id': 2, 'value': (
                    'Cause', ('radioNetwork', 'radio-connection-with-ue-lost')), 'criticality': 'ignore'})
                IEs.append({'id': 26, 'value': (
                    'NAS-PDU', dic['NAS']), 'criticality': 'reject'})
                val.append(('initiatingMessage', {'procedureCode': 16, 'value': (
                    'NASNonDeliveryIndication', {'protocolIEs': IEs}), 'criticality': 'ignore'}))
            else:
                IEs.append({'id': 0, 'value': ('MME-UE-S1AP-ID',
                           dic['MME-UE-S1AP-ID']), 'criticality': 'reject'})
                IEs.append({'id': 8, 'value': ('ENB-UE-S1AP-ID',
                ...
                if dic['SESSION-TYPE'] == "4G" or dic['SESSION-TYPE'] == "5G":
                    ...
                    IEs.append({'id': 67, 'value': ('TAI', {
                               'pLMNidentity': dic['ENB-PLMN'], 'tAC': dic['ENB-TAC-NBIOT']}), 'criticality': 'ignore'})
                val.append(('initiatingMessage', {'procedureCode': 13, 'value': (
                    'UplinkNASTransport', {'protocolIEs': IEs}), 'criticality': 'ignore'}))
            dic = print_log(dic, "S1AP: sending UplinkNASTransport")
        if dic['NAS-SMS-MT'] != None:
            ...
            IEs.append({'id': 8, 'value': ('ENB-UE-S1AP-ID',
                       dic['ENB-UE-S1AP-ID']), 'criticality': 'reject'})
            IEs.append({'id': 26, 'value': (
                'NAS-PDU', dic['NAS-SMS-MT']), 'criticality': 'reject'})
            IEs.append({'id': 100, 'value': ('EUTRAN-CGI', {'cell-ID': (
                dic['ENB-CELLID'], 28), 'pLMNidentity': dic['ENB-PLMN']}), 'criticality': 'ignore'})
            if dic['SESSION-TYPE'] == "4G" or dic['SESSION-TYPE'] == "5G":
                IEs.append({'id': 67, 'value': ('TAI', {
...
def ProcessInitialContextSetupRequest(IEs, dic):
    for i in IEs:
        if i['id'] == 0:
            ...
            for m in range(Num_eRAB):
                ...
                dic['SGW-GTP-ADDRESS'][position] = (
                    first_eRAB['transportLayerAddress'][0]).to_bytes(4, byteorder='big')
            ...
            if len(dic['SGW-GTP-ADDRESS']) > 0:
                ...
                global gtp_dict
                gtp_dict[dic['GTP-KEY']
                         ] = ((dic['SGW-TEID'])[-1], (dic['SGW-GTP-ADDRESS'])[-1])

    val = []

    IEs = []
    IEs.append({'id': 0, 'value': ('MME-UE-S1AP-ID',
               dic['MME-UE-S1AP-ID']), 'criticality': 'ignore'})
    IEs.append({'id': 8, 'value': ('ENB-UE-S1AP-ID',
               dic['ENB-UE-S1AP-ID']), 'criticality': 'ignore'})
    IEs_RABs_List = []
    for m in range(Num_eRAB):
        e_RAB_id = eRAB_list[m]['value'][1]['e-RAB-ID']
        IEs_RAB = {'id': 50, 'value': ('E-RABSetupItemCtxtSURes', {'e-RAB-ID': e_RAB_id, 'transportLayerAddress': (
            dic['ENB-GTP-ADDRESS-INT'], 32), 'gTP-TEID': (struct.pack('>I', upteid_get()))}), 'criticality': 'ignore'}
        IEs_RABs_List.append(IEs_RAB)
        dic['ul-gtp-teid'].append((struct.pack('>I', upteid_get())))
    IEs.append({'id': 51, 'value': ('E-RABSetupListCtxtSURes',
               IEs_RABs_List), 'criticality': 'ignore'})
    val.append(UECapabilityInfoIndication(dic))
    val.append(('successfulOutcome', {'procedureCode': 9, 'value': (
        'InitialContextSetupResponse', {'protocolIEs': IEs}), 'criticality': 'ignore'}))
    ...
    for nas_msg in nas_processed:
        if nas_msg != None:
            ...
            IEs.append({'id': 8, 'value': ('ENB-UE-S1AP-ID',
                       dic['ENB-UE-S1AP-ID']), 'criticality': 'reject'})
            IEs.append({'id': 26, 'value': ('NAS-PDU', nas_msg),
                       'criticality': 'reject'})
            IEs.append({'id': 100, 'value': ('EUTRAN-CGI', {'cell-ID': (
                dic['ENB-CELLID'], 28), 'pLMNidentity': dic['ENB-PLMN']}), 'criticality': 'ignore'})
            if dic['SESSION-TYPE'] == "4G" or dic['SESSION-TYPE'] == "5G":
...
def ProcessERABSetupRequest(IEs, dic):
    ...
    for m in range(Num_eRAB):
        e_RAB_id = eRAB_list[m]['value'][1]['e-RAB-ID']
        IEs_RAB = {'id': 39, 'value': ('E-RABSetupItemBearerSURes', {'e-RAB-ID': e_RAB_id, 'transportLayerAddress': (
            dic['ENB-GTP-ADDRESS-INT'], 32), 'gTP-TEID': (struct.pack('>I', upteid_get()))}), 'criticality': 'ignore'}
        IEs_RABs_List.append(IEs_RAB)
        dic['ul-gtp-teid'].append((struct.pack('>I', upteid_get())))
    IEs.append({'id': 28, 'value': ('E-RABSetupListBearerSURes',
               IEs_RABs_List), 'criticality': 'ignore'})
    val.append(('successfulOutcome', {'procedureCode': 5, 'value': (
        'E-RABSetupResponse', {'protocolIEs': IEs}), 'criticality': 'ignore'}))
...
def ProcessERABReleaseCommand(IEs, dic):
    ...
    for m in range(Num_eRAB):
        e_RAB_id = eRAB_list[m]['value'][1]['e-RAB-ID']
        IEs_RAB = {'id': 15, 'value': (
            'E-RABReleaseItemBearerRelComp', {'e-RAB-ID': e_RAB_id}), 'criticality': 'ignore'}
        IEs_RABs_List.append(IEs_RAB)

    IEs.append({'id': 69, 'value': ('E-RABReleaseListBearerRelComp',
               IEs_RABs_List), 'criticality': 'ignore'})
    val = ('successfulOutcome', {'procedureCode': 7, 'value': (
        'E-RABReleaseResponse', {'protocolIEs': IEs}), 'criticality': 'ignore'})
    dic = print_log(dic, "S1AP: sending ERABSetupResponse")
    ...
    if dic['NAS'] != None:
        ...
        if dic['SESSION-TYPE'] == "4G" or dic['SESSION-TYPE'] == "5G":
            ...
            IEs.append({'id': 67, 'value': ('TAI', {
                       'pLMNidentity': dic['ENB-PLMN'], 'tAC': dic['ENB-TAC-NBIOT']}), 'criticality': 'ignore'})
        val2 = ('initiatingMessage', {'procedureCode': 13, 'value': (
            'UplinkNASTransport', {'protocolIEs': IEs}), 'criticality': 'ignore'})
                            ...
                            for recv_id in id_pair['value']:
                                if isinstance(recv_id, tuple):
                                    if isinstance(recv_id[1], dict):
                                        IEs.append({'id': 0, 'value': (
                                            'MME-UE-S1AP-ID', recv_id[1]['mME-UE-S1AP-ID']), 'criticality': 'ignore'})
                                        IEs.append({'id': 8, 'value': (
                                            'ENB-UE-S1AP-ID', recv_id[1]['eNB-UE-S1AP-ID']), 'criticality': 'ignore'})

    val = ('successfulOutcome', {'procedureCode': 23, 'value': (
        'UEContextReleaseComplete', {'protocolIEs': IEs}), 'criticality': 'ignore'})
...
def ProcessUEContextModificationRequest(IEs, dic):
    ...
    val.append(('successfulOutcome', {'procedureCode': 21, 'value': (
        'UEContextModificationResponse', {'protocolIEs': IEs}), 'criticality': 'ignore'}))
        ...
        if i['id'] == 43:
            ...
            if i['value'][1] == 'ps' and SEND_NAS == True:
                if dic['SESSION-TYPE'] == "4G" or dic['SESSION-TYPE'] == "5G":
                    ...
                    dic['CPSR-TYPE'] = 1
                    dic = ProcessUplinkNAS(
                        'control plane service request', dic)
...
def UEContextReleaseRequest(dic):
    ...
    if dic['UECONTEXTRELEASE-CSFB'] == False:
        IEs.append({'id': 2, 'value': ('Cause', ('radioNetwork',
                   'user-inactivity')), 'criticality': 'ignore'})
    else:
        IEs.append({'id': 2, 'value': ('Cause', ('radioNetwork',
                   'cs-fallback-triggered')), 'criticality': 'ignore'})

    val = ('initiatingMessage', {'procedureCode': 18, 'value': (
        'UEContextReleaseRequest', {'protocolIEs': IEs}), 'criticality': 'ignore'})
...
def UECapabilityInfoIndication(dic):
    ...
    IEs.append({'id': 8, 'value': ('ENB-UE-S1AP-ID',
               dic['ENB-UE-S1AP-ID']), 'criticality': 'ignore'})
    IEs.append({'id': 74, 'value': ('UERadioCapability', ue_cap),
               'criticality': 'ignore'})
    val = ('initiatingMessage', {'procedureCode': 22, 'value': (
        'UECapabilityInfoIndication', {'protocolIEs': IEs}), 'criticality': 'ignore'})
...
def ERABModificationIndication(dic):
    ...
    for i in range(len(dic['RAB-ID'])):
        RABList.append({'id': 200, 'value': ('E-RABToBeModifiedItemBearerModInd', {'e-RAB-ID': dic['RAB-ID'][i], 'transportLayerAddress': (
            dic['ENB-GTP-ADDRESS-INT'], 32), 'dL-GTP-TEID': b'\x00\x00\x00' + bytes([dic['RAB-ID'][i]])}), 'criticality': 'ignore'})
    IEs.append({'id': 199, 'value': (
        'E-RABToBeModifiedListBearerModInd', RABList), 'criticality': 'reject'})
    ...
    b = {'id': 267, 'value': ('E-RABUsageReportItem', {'startTimestamp': startTimestamp, 'endTimestamp': endTimestamp,
                              'usageCountUL': 12345678, 'usageCountDL': 9876543}), 'criticality': 'ignore'}

    SecondaryRATDataList = []
    for i in range(len(dic['RAB-ID'])):
        SecondaryRATDataList.append({'id': 265, 'value': ('SecondaryRATDataUsageReportItem', {
                                    'e-RAB-ID': dic['RAB-ID'][i], 'secondaryRATType': 'nR', 'e-RABUsageReportList': [b]}), 'criticality': 'ignore'})

    IEs.append({'id': 264, 'value': ('SecondaryRATDataUsageReportList',
               SecondaryRATDataList), 'criticality': 'ignore'})

    dic = print_log(dic, "S1AP: sending E-RABModificationIndication")
    val = ('initiatingMessage', {'procedureCode': 50, 'value': (
        'E-RABModificationIndication', {'protocolIEs': IEs}), 'criticality': 'reject'})
...
def SecondaryRATDataUsageReport(dic):
    ...
    val = ('initiatingMessage', {'procedureCode': 62, 'value': (
        'SecondaryRATDataUsageReport', {'protocolIEs': IEs}), 'criticality': 'reject'})
...
def send_gtpu(session_d):
    try:
        hex_str = bytes.fromhex('30ff0064') + session_d['SGW-TEID'][0] + bytes.fromhex('4500006493d9000040015652') + session_d['PDN-ADDRESS'][0][1:] + bytes.fromhex(
            '0808080808006ace039600015635c5637dfc0b0008090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f202122232425262728292a2b2c2d2e2f303132333435363738393a3b3c3d3e3f4041424344454647')
        gtp = hex_str
        pkt = IP(src="192.168.197.180", dst="192.168.197.201") / \
            UDP(sport=2152, dport=2152)/Raw(gtp)
        for i in range(10):
...
def ProcessS1AP(type, pdu_dict, client, session_dict):
    ...
    if type == 'initiatingMessage':
        ...
        if procedure == 'MMEConfigurationUpdate':
            session_dict = print_log(
                session_dict, "S1AP: MMEConfigurationUpdate received")
            answer, session_dict = MMEConfigurationUpdateAcknowledge(
                protocolIEs_list, session_dict)
            PDU.set_val(answer)
            message = PDU.to_aper()
        ...
        # UE Related:
            ...
            session_dict = print_log(
                session_dict, "S1AP: DownlinkNASTransport received")
            answer_list, session_dict = ProcessDownlinkNASTransport(
                protocolIEs_list, session_dict)
            ...
            session_dict = print_log(
                session_dict, "S1AP: InitialContextSetupRequest received")
            answer_list, session_dict = ProcessInitialContextSetupRequest(
                protocolIEs_list, session_dict)
            for answer in answer_list:
                if answer != None:
                    PDU.set_val(answer)
            ...
            session_dict = print_log(
                session_dict, "S1AP: UEContextReleaseCommand received")
            answer, session_dict = ProcessUEContextReleaseCommand(
                pdu_dict, protocolIEs_list, session_dict)
            if answer != None:
                PDU.set_val(answer)
                message = PDU.to_aper()
                bytes_sent = client.send(message)
            ...
            if session_dict['PROCESS-PAGING'] == True:
                ...
                answer, session_dict = ProcessPaging(
                    protocolIEs_list, session_dict)
                if answer != None:
                    PDU.set_val(answer)
                    message = PDU.to_aper()
            ...
            session_dict = print_log(
                session_dict, "S1AP: ERABSetupRequest received")
            answer_list, session_dict = ProcessERABSetupRequest(
                protocolIEs_list, session_dict)
            for answer in answer_list:
                if answer != None:
                    PDU.set_val(answer)
            ...
            session_dict = print_log(
                session_dict, "S1AP: ERABReleaseCommand received")
            answer_list, session_dict = ProcessERABReleaseCommand(
                protocolIEs_list, session_dict)
            for answer in answer_list:
                if answer != None:
                    PDU.set_val(answer)
            ...
            session_dict = print_log(
                session_dict, "S1AP: LocationReportingControl received")
            answer, session_dict = ProcessLocationReportingControl(
                protocolIEs_list, session_dict)
            if answer != None:
                PDU.set_val(answer)
                message = PDU.to_aper()
            ...
            session_dict = print_log(
                session_dict, "S1AP: UEContextModificationRequest received")
            ...
            session_dict = print_log(
                session_dict, "S1AP: " + procedure + " received")

    elif type == 'successfulOutcome':
        procedure, protocolIEs_list = pdu_dict['value'][0], pdu_dict['value'][1]['protocolIEs']
        if procedure == "S1SetupResponse":
            session_dict = print_log(
                session_dict, "S1AP: S1SetupResponse received")
            session_dict = S1SetupResponseProcessing(
                protocolIEs_list, session_dict)

        elif procedure == "ResetAcknowledge":
            session_dict = print_log(
                session_dict, "S1AP: ResetAcknowledge received")
        else:
            session_dict = print_log(
                session_dict, "S1AP: " + procedure + " received")
...
def open_tun(n):
    ...
    if sys.platform == "linux" or sys.platform == "linux2":
        ...
        ifs = fcntl.ioctl(f, TUNSETIFF, struct.pack(
            "16sH", bytes("tun%d" % n, "utf-8"), TUNMODE))
    ...
    while True:
        try:
            ...
            if tap_packet[0][26:30] in gtp_dict:
                ...
                s_gtpu.sendto(gtp_u_header(teid, len(
                    tap_packet[0][14:])) + tap_packet[0][14:], (socket.inet_ntoa(gtp_dict[tap_packet[0][26:30]][1]), 2152))
        except:
            pass
    ...
    while True:
        ...
        if gtp_packet[0:2] == b'\x30\xff':
            if gtp_packet[24:28] in down_gtp:
                ethHeader = Ether(src="ee:ee:ee:ee:ee:ee",
                                  dst=down_gtp[gtp_packet[24:28]][0], type=0x0800)
            ...
            s_gtpu.sendto(gtp_echo_response, (socket.inet_ntoa(
                session_dict['SGW-GTP-ADDRESS'][-1]), 2152))
...
class UserDict(dict):
    def __init__(self, *arg, **kw):
        ...
        self.setdefault(
            'NON-IP-PACKETS', [NON_IP_PACKET_1, NON_IP_PACKET_2, NON_IP_PACKET_3, NON_IP_PACKET_4])
...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option("-i", "--enb_ip", dest="enb_ip",
                      help="eNB Local IP Address")
    parser.add_option("-m", "--mme_ip", dest="mme_ip", help="MME IP Address")
    (options, args) = parser.parse_args()
    options.serial_interface = "/dev/ttyUSB2"

    server_address = (options.mme_ip, S1AP_PORT)

    # socket options
    if (type(ip_address(options.mme_ip))) is IPv6Address:
        client = socket.socket(
            socket.AF_INET6, socket.SOCK_STREAM, socket.IPPROTO_SCTP)
        s_gtpu = socket.socket(socket.AF_INET6, socket.SOCK_DGRAM)
        interface_name = [interface for interface in netifaces.interfaces() if 10 in netifaces.ifaddresses(
            interface) for name in netifaces.ifaddresses(interface)[10] if name['addr'] == str(options.enb_ip)][0]
    else:
        client = socket.socket(
    ...
    try:
        ...
        logger.info(f"enb ip error {e} {options.enb_ip}")
        sys.exit()
    ...
    try:
        ...
        logger.error(f"Unable to connect to MME: {e} {server_address}")
        sys.exit()
        ...
        if read_sockets:
            for sock in read_sockets:
                if sock == client:
                    ...
                    (pdu_type, pdu_dict) = PDU()
                    try:
                        session_dict = user_dict[[user[0] for user in list(user_dict.items()) if user[1]['ENB-UE-S1AP-ID'] == int([ies['value'][1] for d in pdu_dict['value'] if isinstance(
                            d, dict) for ies in d['protocolIEs'] if isinstance(ies, dict) if ies['id'] == 8 if ies['value'][0] == 'ENB-UE-S1AP-ID'][0])][0]]
                    except:
                        pass
                    ...
                    PDU, client, session_dict = ProcessS1AP(
                        pdu_type, pdu_dict, client, session_dict)
        ...
        if q.qsize() > 0:
            ...
            if queue_msg['procedure'] == 's1-setup':
                ...
                if 'tac1' in queue_msg:
                    session_dict['ENB-TAC1'] = int(queue_msg['tac1']
                                                   ).to_bytes(2, byteorder='big')
                else:
                    session_dict['ENB-TAC1'] = int(63).to_bytes(2,
                                                                byteorder='big')
                ...
                if queue_msg['imsi'] in user_dict:
                    logger.info(
                        f'*************** imsi {queue_msg} found in object')
                    ...
                    try:
                        del gtp_dict[hexlify(socket.inet_ntoa(
                            session_dict['PDN-ADDRESS-IPV4']))]
                    ...
                    if set(('imsi', 'ki', 'opc')).issubset(queue_msg) and 'enb' in user_dict:
                        ...
                        session_dict['SGW-TEID'] = []
                        session_dict['EPS-BEARER-IDENTITY'] = []
                        # default 0, dedicated 1
                        session_dict['EPS-BEARER-TYPE'] = []
                        # active 1, inactive 0
                        session_dict['EPS-BEARER-STATE'] = []
                        session_dict['EPS-BEARER-APN'] = []
                        session_dict['ENCODED-IMSI'] = eNAS.encode_imsi(
                            session_dict['IMSI'])
                        session_dict['MOBILE-IDENTITY'] = session_dict['ENCODED-IMSI']
                        session_dict['ENCODED-IMEI'] = eNAS.encode_imei(
                            session_dict['IMEISV'])
                        session_dict['ENCODED-GUTI'] = eNAS.encode_guti(
                            session_dict['PLMN'], 32769, 1, 12345678)
                        session_dict['KASME'] = b'kasme   kasme   kasme   kasme   '
                        ...
                        session_dict['NAS-KEY-EIA3'] = return_key(
                            session_dict['KASME'], 3, 'NAS-INT')
                        session_dict['ENB-GTP-ADDRESS-INT'] = ip2int(
                            options.enb_ip)
                        ...
                        if 'attach_type' in queue_msg and queue_msg['attach_type'] == 'guti':
                            session_dict['GUTI'] = unhexlify(
                                'f613051000010100000000')
                    else:
                        if 'enb' not in user_dict:
                            logger.error(
                                f"*************** S1-Setup is not done")
                            break
                        logger.info(
                            f"*************** {queue_msg['procedure']} not matching")
                        break
            msg = queue_msg['procedure']
            logger.info(gtp_dict)
            if msg == 's1-setup':
                PDU, client, session_dict = eMENU.ProcessMenu(
                    PDU, client, session_dict, msg, user_dict)
            elif 'IMSI' in session_dict:
                if msg == "active-users":
                    logger.info(
                        f'*************** Active IMSIs :- {[user for user in list(user_dict.keys()) if user != "enb"]}')
                else:
                    logger.info(f"Sending message \"{msg}\" to ProcessMenu")
                    PDU, client, session_dict = eMENU.ProcessMenu(
                        PDU, client, session_dict, msg, user_dict)
            else:
                logger.info(
                    f"*************** {msg} no if active user in tool {len(user_dict)} no of gtp tunnel {len(gtp_dict)}")
